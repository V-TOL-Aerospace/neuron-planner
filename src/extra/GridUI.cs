...
namespace MissionPlanner.Grid
{
    public partial class GridUI : Form
    {
        ...
        private void GridUI_Load(object sender, EventArgs e)
        {
            ...
            label1.Text += " (" + CurrentState.AltUnit +")";
        ...
        private async void domainUpDown1_ValueChanged(object sender, EventArgs e)
        {
            ...
            {
                grid = Utilities.Grid.CreateCorridor(list, CurrentState.fromAltDisplayUnit((double)NUM_altitude.Value),
            ...
            {
                grid = await Utilities.Grid.CreateRotaryAsync(list, CurrentState.fromAltDisplayUnit((double)NUM_altitude.Value),
                    (double)NUM_Distance.Value, (double)NUM_spacing.Value, (double)NUM_angle.Value,
                    (double)NUM_overshoot.Value, (double)NUM_overshoot2.Value,
                    (Utilities.Grid.StartPosition)Enum.Parse(typeof(Utilities.Grid.StartPosition), CMB_startfrom.Text), false,
            ...
            {
                grid = await Utilities.Grid.CreateGridAsync(list, CurrentState.fromAltDisplayUnit((double)NUM_altitude.Value),
                    (double)NUM_Distance.Value, (double)NUM_spacing.Value, (double)NUM_angle.Value,
                    (double)NUM_overshoot.Value, (double)NUM_overshoot2.Value,
                    (Utilities.Grid.StartPosition)Enum.Parse(typeof(Utilities.Grid.StartPosition), CMB_startfrom.Text), false,
                    (float)NUM_Lane_Dist.Value, (float)NUM_leadin.Value, MainV2.comPort.MAV.cs.HomeLocation);
            ...
            if (chk_crossgrid.Checked)
            {
                ...
                grid.AddRange(Utilities.Grid.CreateGrid(list, CurrentState.fromAltDisplayUnit((double) NUM_altitude.Value),
            ...
            */
            ...
            if (DistUnits == "Feet") // Keep Metric
            {
                ...
                float area = (float)calcpolygonarea(list);
                if (area > 99999)
                    lbl_area.Text = (area/1000000).ToString("0,0.00", System.Globalization.CultureInfo.InvariantCulture) + " km^2";
                else
                    lbl_area.Text = area.ToString("0,0.00", System.Globalization.CultureInfo.InvariantCulture) + " m^2";
                
                lbl_distance.Text = routetotal.ToString("0,0.00", System.Globalization.CultureInfo.InvariantCulture) + " km";
                lbl_spacing.Text = NUM_spacing.Value.ToString("0,0.00", System.Globalization.CultureInfo.InvariantCulture) + " m";
            ...
            {
                // Meters
                lbl_area.Text = calcpolygonarea(list).ToString("0,0.00", System.Globalization.CultureInfo.InvariantCulture) + " m^2";
            ...
            // update Flight Plan Info
            FlightData.secEFT = seconds/0.8; // reduce flying speed by 20% to add 20% buffer to estimation. 
            FlightData.estFltDist = lbl_distance.Text;
            FlightData.estSpacing = lbl_distbetweenlines.Text;

            FlightData.numImg = images;
            FlightData.secPerImg = lbl_photoevery.Text;
            FlightData.estGSD = TXT_cmpixel.Text;
            FlightData.numStrips = ((int)(strips / 2)).ToString();
        ...
        void AddDrawPolygon()
        {
            ...
            {
                GMarkerGoogle m = new GMarkerGoogle(item, GMarkerGoogleType.red);
                m.ToolTipMode = MarkerTooltipMode.Never;
                GMapMarkerRect mBorders = new GMapMarkerRect(item);
                {
                    mBorders.InnerMarker = m;
                    mBorders.wprad = 5;
                }
                routesOverlay.Markers.Add(m);
                routesOverlay.Markers.Add(mBorders);
            }
        }
        ...
        void doCalc()
        {
            try
            {
                ...
                float flyalt = (float)CurrentState.fromAltDisplayUnit((float)NUM_altitude.Value);
        ...
        private void map_OnMarkerLeave(GMapMarker item)
        {
            if (!isMouseDown)
            {
                ...
                {
                    GMapMarkerRect polymarker = item as GMapMarkerRect;
                    polymarker.ResetColor();
        ...
        private void map_OnMarkerEnter(GMapMarker item)
        {
            if (!isMouseDown)
            {
                ...
                {
                    GMapMarkerRect polymarker = item as GMapMarkerRect;
                    polymarker.Pen.Color = Color.Red;
                    CurrentGMapMarker = polymarker;
                }
                if (item is GMapMarker)
                {
        ...
        private void map_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left && isMouseDown)
            {
                ...
                //XXX: Removed left click pan, must use mouse
                else // left click pan
                {
        ...
        private void BUT_Accept_Click(object sender, EventArgs e)
        {
            if (grid != null && grid.Count > 0)
            {
                ...
                for (int splitno = 0; splitno < NUM_split.Value; splitno++)
                {
                    ...
                    // XXX: seperated Use RTL from add Takeoff and Land WPs. This will facilitate adding RTL for Grid surveying with Multi-rotor aircraft.  
                    if (CHK_toandland_RTL.Checked)
                    {
                        plugin.Host.AddWPtoList(MAVLink.MAV_CMD.RETURN_TO_LAUNCH, 0, 0, 0, 0, 0, 0, 0, gridobject);
                    }
